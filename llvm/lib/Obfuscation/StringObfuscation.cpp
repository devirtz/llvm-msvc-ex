...
static cl::opt<bool>
    RunStringObfuscationPass("string-obfus", cl::init(false),
                             cl::desc("OLLVM - StringObfuscationPass"));
...
struct StringEncryption {
  ...
  StringEncryption() = default;

  ~StringEncryption() {
    //outs() << "End StringEncryption\n";
    for (const CSPEntry *entry : ConstantStringPool) {
      delete (entry);
    }
    for (const auto &i : CSUserMap) {
      const CSUser *user = i.second;
      delete (user);
    }
    //outs() << "End StringEncryption ok1\n";
    ConstantStringPool.clear();
    CSPEntryMap.clear();
    CSUserMap.clear();
    MaybeDeadGlobalVars.clear();
   // outs() << "End StringEncryption ok2n";
  }
...
bool StringEncryption::processConstantStringUse(Function *F) {
  if (!toObfuscate(RunStringObfuscationPass, F, "string-obfus")) {
    outs() << "no string obfus\n";
    return false;
  }
  //
  LowerConstantExpr(*F);
...
PreservedAnalyses StringObfuscationPass::run(Module &M,
  ...
  if (RunStringObfuscationPass == true) {
